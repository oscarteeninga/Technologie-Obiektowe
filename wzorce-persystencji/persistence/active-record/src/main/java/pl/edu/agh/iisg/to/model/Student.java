...
public class Student {
	...
	public static Optional<Student> create(final String firstName, final String lastName, final int indexNumber) {
		String insertSql = String.format("INSERT INTO %s (first_name, last_name, index_number) VALUES ('%s', '%s', %d);", TABLE_NAME, firstName, lastName, indexNumber);
		try {
			final int id = QueryExecutor.createAndObtainId(insertSql);
	...
	public static Optional<Student> findByIndexNumber(final int indexNumber) {
		String findByIndexNumberSql = String.format("SELECT * FROM %s WHERE index_number = %d", TABLE_NAME, indexNumber);
		try {
